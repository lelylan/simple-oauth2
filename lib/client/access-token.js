...
module.exports = (config) => {
  ...
  */
  AccessToken.prototype.refresh = async function refresh(params) {
    const options = Object.assign({}, params, {
      grant_type: 'refresh_token',
      refresh_token: this.token.refresh_token,
    });

    const response = await core.api('POST', config.auth.tokenPath, options);
  ...
  */
  AccessToken.prototype.revoke = function revoke(tokenType) {
    ...
    return core.api('POST', config.auth.revokePath, options);
  };
