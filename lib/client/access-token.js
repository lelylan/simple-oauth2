...
module.exports = (config) => {
  ...
  const refreshUrl = config.auth.refreshPath ? url.resolve(config.auth.tokenHost, config.auth.refreshPath) : tokenUrl;
  ...
  */
  AccessToken.prototype.refresh = function refresh(params, callback) {
    ...
    return core
      .api('POST', refreshUrl, options)
      .then((response) => createAccessToken(response))
      .nodeify(callback);
  };

    /**
  * Refresh the access token for Philips Hue
  * @param  {Object} An optional argument for additional API request params.
  * @param  {Function} callback
  */
  AccessToken.prototype.refresh_hue = function refresh(params, callback) {
    if (typeof params === 'function') {
      callback = params;
      params = undefined;
    }
    ...
    return core
      .api('POST', refreshUrl+'?grant_type=refresh_token', options)
      .then((response) => createAccessToken(response))
      .nodeify(callback);
  };
